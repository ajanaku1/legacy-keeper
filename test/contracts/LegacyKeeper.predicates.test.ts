...
describe("LegacyKeeper — Goal.md predicates", () => {
  ...
  describe("Liveness resets the clock", () => {
    it("allows at most one heartbeat during a rolling 24-hour window", async () => {
      ...
      const recordedAt = (await keeper.liveness()).lastHeartbeat;
      await time.setNextBlockTimestamp(Number(recordedAt) + DAY - 1);
      await expect(keeper.heartbeat()).to.be.revertedWith(
        "LK: heartbeat cooldown",
      );
      await time.setNextBlockTimestamp(Number(recordedAt) + DAY);
      await expect(keeper.heartbeat()).to.emit(keeper, "HeartbeatRecorded");
    });
  ...
  describe("B1/B2 · evacuation", () => {
    it("sweeps native ETH to the vault on a recovery-key signature alone", async () => {
      ...
      expect(
        (await ethers.provider.getBalance(vault.address)) - before,
      ).to.equal(ONE_ETH);
    });
    ...
    it("rejects a signature from the owner key — only the recovery key authorizes", async () => {
      ...
      await expect(keeper.evacuate(3, deadline, ownerSig)).to.be.revertedWith(
        "LK: invalid signature",
      );
    });
    ...
    it("rejects a malleable signature", async () => {
      ...
      await expect(keeper.evacuate(4, deadline, flipped)).to.be.revertedWith(
  ...
  describe("B5 · the two modes are mutually exclusive", () => {
    ...
    it("inheritance blocks a later evacuation", async () => {
      ...
      expect(
        await ethers.provider.getBalance(await keeper.getAddress()),
      ).to.equal(0n);
    });
  });
  ...
  describe("Review findings — regressions", () => {
    ...
    it("distributes a USDT-shaped token that returns no data", async () => {
      ...
      expect(await usdt.balanceOf(b1.address)).to.equal(
        (supply * 6000n) / 10000n,
      );
      expect(await usdt.balanceOf(b2.address)).to.equal(
        (supply * 4000n) / 10000n,
      );
  ...
  describe("One hostile beneficiary cannot brick the estate", () => {
    it("credits a pull balance when a push fails and pays everyone else", async () => {
      ...
      expect(await keeper.pendingWithdrawal(hostileAddr)).to.equal(
        ONE_ETH / 2n,
      );
    });
  });
