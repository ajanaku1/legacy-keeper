...
export function buildPlanCreationRequest(
  ...
  return {
    ...
    owner: draft.owner,
    config: {
      heartbeatInterval: SECONDS_PER_DAY,
      timeoutDuration: draft.timeoutDays * SECONDS_PER_DAY,
      gracePeriod: draft.graceDays * SECONDS_PER_DAY,
  ...
  if (!response.ok || !isPlanEvidence(body)) {
    throw new Error(
      userFacingActionError(
        body,
        'KeeperHub could not verify plan creation.'
      )
    );
  }
  return body;
...
export function randomNonce(bytes: Uint8Array): bigint {
  if (bytes.length !== 32) throw new Error('Nonce requires exactly 32 random bytes.');
  let nonce = 0n;
