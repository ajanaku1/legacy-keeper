...
export async function submitHeartbeatWebhook(
  ...
  if (!response.ok) throw new Error(`KeeperHub webhook failed: HTTP ${response.status}`);
  return { executionId: parseWebhookExecutionId(body) };
}

export const submitSignedWorkflowWebhook = submitHeartbeatWebhook;
...
export function parseWebhookExecutionId(value: unknown): string {
  ...
  if (!executionId) throw new Error('KeeperHub webhook returned no execution ID');
  return executionId;
}

export function parseKeeperHubExecution(value: unknown): KeeperHubSettlement {
  const root = asObject(value);
  const execution = asObject(asObject(root.logs).execution);
  ...
  return {
    ...
    txHash: transactionHash(hashes[0]),
    sponsored: execution.sponsored === true || output.sponsored === true,
  };
}

function transactionHash(value: unknown): `0x${string}` | undefined {
  const hash = stringValue(asObject(value).hash);
