...
describe('wallet-scoped activity', () => {
  it('returns only entries belonging to the requested wallet', () => {
    const entries = parseAuditLedger([
      JSON.stringify({ executionKey: `heartbeatBySig:${OWNER_A}:1`, owner: OWNER_A }),
      JSON.stringify({ executionKey: `createPlan:${OWNER_B}:2`, owner: OWNER_B }),
  ...
  it('redacts server errors and internal execution identifiers', () => {
    const entries = parseAuditLedger(
      JSON.stringify({
  ...
  it('returns five newest wallet activities per page', () => {
    ...
    expect(last).toMatchObject({ page: 3, pageSize: 5, total: 12, totalPages: 3 });
