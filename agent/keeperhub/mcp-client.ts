...
export class McpClient {
  ...
  async callToolJson<T = unknown>(
    ...
    try {
      ...
      throw new McpError(`tool ${name} returned non-JSON: ${text.slice(0, 200)}`);
    }
  }
      ...
      try {
        ...
        if (body?.error) {
          ...
          if (code === -32003) {
            this.sessionId = null;
            await this.connect();
            throw new McpError(message, code, true);
          }
  ...
  private async post(payload: unknown): Promise<{ response: Response; body: any }> {
