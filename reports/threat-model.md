...
| S1b | `setSafeVault` was owner-callable at any time. Redirecting the vault is equivalent to theft — evacuation would sweep straight to the attacker. | **Critical** | Owner-settable only until a recovery key exists; thereafter requires a recovery-key signature (`setSafeVaultBySig`). |
...
| S5 | Unbounded beneficiary array could be configured past the block gas limit, making the estate permanently undistributable. | **Medium** | `MAX_BENEFICIARIES = 20`. |
...
- **Native ETH must be deposited.** An EOA's native balance cannot be pulled,
  ...
  the UI and README rather than engineered around.
